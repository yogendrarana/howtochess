...
export function ChessBoard() {
	...
	const [moveHistory, setMoveHistory] = useState<MoveHistory[]>([
		{ fen: game.fen(), move: "", san: "Start" },
	]);
	const [currentMoveIndex, setCurrentMoveIndex] = useState(0);
	const [isBoardFlipped, setIsBoardFlipped] = useState(false);
	const [playerColor, setPlayerColor] = useState<"both" | "white" | "black">(
		"both",
	);
	...
	useEffect(() => {
		...
		moveHistory.forEach((history) => {
			if (history.captured) {
				const color =
					history.captured === history.captured.toLowerCase() ? "b" : "w";
						...
						<select
							...
							onChange={(e) =>
								handlePlayerColorChange(
									e.target.value as "both" | "white" | "black",
								)
							}
										...
										displayFiles.map((file) => {
											...
											return (
												<Square
													...
													isLastMove={
														lastMove?.from === id || lastMove?.to === id
													}
									...
									{Array.from({
										...
										return (
											<tr
												...
												onClick={() =>
													goToMove(blackMove ? index * 2 + 2 : index * 2 + 1)
												}
											>
...
function Square({
	...
	onClick,
	isBoardFlipped,
}: SquareProps & { onClick: () => void; isBoardFlipped: boolean }) {
